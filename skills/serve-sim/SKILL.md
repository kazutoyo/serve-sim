...
| Take a screenshot | `npx serve-sim screenshot` | Saves PNG to `--captures-dir` (default `./serve-sim-captures`); copies to clipboard when possible. |
| Start recording | `npx serve-sim record start` | Begins MP4 screen recording via `simctl io recordVideo`. |
| Stop recording | `npx serve-sim record stop` | Finalizes the MP4 and prints the saved path. |
